REGION=us-west-2
CONFIG_NAME=1121-dorm-mix-2node-p4de
...
for queue in "${!queues[@]}"; do
    ...
    NUM_NODES=2
    NUM_GPUS_PER_NODE=8
    NUM_CPUS_PER_NODE=96  # Changed from 96 for P5
    MEMORY_PER_NODE=1143265  # Changed from 1143265 for P5
    NETWORK_DEVICE_NUM=4   # Changed from 4 for P5
    ...
    JOB_DEFINITION_NAME=nemo-aligner-${random_job_suffix}-${JOB_NAME}
    JOB_TYPE=multinode
...
#    RUNCMD_LIST_STR=$(printf '%s\n' "${RUNCMD_LIST[@]}" | jq -R . | jq -s .)
    ...
    RUNCMD_LIST=("sh" "-c" "${RUNCMD}")
    RUNCMD_LIST_STR=$(echo '[' $(printf '"%s",' "${RUNCMD_LIST[@]}" | sed 's/,$//') ']')
