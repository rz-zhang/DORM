REGION=us-east-1
CONFIG_NAME=12b_rm_hs2_20k_3epoch_1022
export GPFS="/fsx-Training/shopqa-training-fsx-prod-${REGION}/home/ronzhi/12b/NeMo-Aligner"

declare -A queues
queues[FS-P4D_24XL-Training-us-east-1]=p4d.24xlarge
...
for queue in "${!queues[@]}"; do
    ...
    JOB_PRIORITY=100
    SHARE_IDENTIFIER=Smaller4NodeTraining
#    MLFLOW_TRACKING_URI=https://prod.$REGION.internal.mlflow.nile.amazon.dev
    ...
    RUNCMD="ls -l && ls -l ${GPFS} && cd ${GPFS} && ${DDP_ENVS} ./batch_rm_hs2_train.sh"
...
#   RUNCMD_LIST=("sh" "-c" "${DDP_ENVS} ${RUNCMD} ${RUN_ARGS}")
    ...
    CONTAINER_PROPERTIES='{
        ...
        {
            "host": {
                "sourcePath": "/fsx-Training/home/$USER/Mistral-NeMo-12B-Instruct"
            },
            "name": "12b-model-volume"
        }
        ],
        "mountPoints": [
            {
            ...
            {
                "sourceVolume": "12b-model-volume",
                "containerPath": "/workspace/Mistral-NeMo-12B-Instruct",
                "readOnly": false
            }
        ],
        "linuxParameters": {
    ...
    NODE_PROP_STR=$(echo "$NODE_PROP_STR" | jq -c '.')
